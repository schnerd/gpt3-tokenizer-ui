// Source from AI Dungeon GPT-3-Encoder https://github.com/AIDungeon/GPT-3-Encoder
// (slightly modified to return token text)

export function createTokenizer(encoder, bpe_file) {
  const range = (x, y) => {
...
// bpe_merges = [tuple(merge_str.split()) for merge_str in bpe_data.split("\n")[1:-1]]
  ...
  function encode(text) {
    ...
    for (let token of matches) {
      ...
      tokens = tokens.concat(new_tokens.map(token => decode([token])));
    }
